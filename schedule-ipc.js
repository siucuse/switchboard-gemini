...
const SCHEDULE_COMMANDS_DIR = path.join(GEMINI_DIR, 'commands');

const SCHEDULE_CREATOR_TEMPLATE = `---
name: create-switchboard-schedule
description: Create a new SwitchboardGemini scheduled task for this project
---

You are helping the user create a scheduled task in SwitchboardGemini. This task will run automatically on a cron schedule using Gemini CLI CLI in headless mode (-p flag).
...
Save to \`<project-root>/.gemini/commands/schedule-<slug>.md\`:
...
<The full prompt that will be sent to Gemini when this task runs>
\`\`\`
...
- If the \`.gemini/commands/\` directory doesn't exist, create it
- After saving, tell the user: "Your scheduled task is saved! It will appear in SwitchboardGemini's brain tab with a schedule icon. You can enable/disable it or edit the schedule from there."
- If the user wants to see existing schedules, list any \`schedule-*.md\` files in \`.gemini/commands/\`
`;

const SCHEDULE_WELCOME_MESSAGE = `## SwitchboardGemini Scheduled Task Creator

Welcome! This session will help you create a **scheduled task** that runs automatically on a cron schedule using Gemini CLI.
...
- The schedule file gets saved to this project's \`.gemini/commands/\` directory as a command — so it can also be run manually from any Gemini session using \`/schedule-<name>\`
- Once saved, it appears in the **brain tab** with a clock icon where you can edit it directly
- To edit, you can also ask use this schedule gemini session to ask to edit existing commands.
- SwitchboardGemini runs matching schedules automatically in the background — each run creates a session grouped under the task's slug

### What you can configure
- **The prompt** — what Gemini should do each time the task runs
...
function init(log, runCommand) {
  ...
  ipcMain.handle('create-schedule-session', (_event, projectPath) => {
    try {
      ...
      const geminiProjectDir = path.join(PROJECTS_DIR, folder);
  ...
  ipcMain.handle('run-schedule-now', (_event, filePath) => {
    try {
      ...
      const dotGeminiDir = path.dirname(commandsDir);
      const projectPath = path.dirname(dotGeminiDir);
