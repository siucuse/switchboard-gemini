...
//   1. OSC 0 braille spinner (authoritative: Gemini CLI sets title to spinner chars)
...
const responseReadySessions = new Set(); // Gemini finished, user hasn't looked (terminal state)
...
window.api.onPlanDetected(async (sessionId, filePath) => {
  log.info(`[plan-detected] session=${sessionId} path=${filePath}`);
  
  // If this is the active session, notify the user or auto-open
  if (sessionId === activeSessionId) {
    // Show a notification in the header
    if (terminalHeaderPtyTitle) {
      terminalHeaderPtyTitle.textContent = 'New Plan Available';
      terminalHeaderPtyTitle.style.display = '';
      terminalHeaderPtyTitle.classList.add('flash');
      setTimeout(() => terminalHeaderPtyTitle.classList.remove('flash'), 2000);
    }
    
    // Auto-open in side panel
    const filename = filePath.split('/').pop();
    const result = await window.api.readPlan(filename, sessionId);
    if (result && result.content) {
      if (typeof openPlanTab === 'function') {
        openPlanTab(sessionId, { filename, filePath, content: result.content });
      }
    }
  }
  // Refresh plans list if plans tab is active
  if (activeTab === 'plans') {
    loadPlans();
  }
});
...
window.api.onTerminalNotification((sessionId, message) => {
  ...
  // 1. "Gemini CLI needs your attention"         → attention
  // 2. "Gemini CLI needs your approval for the plan" → approval, needs your
  // 3. "Gemini needs your permission to use {tool}"   → permission, needs your
  // 4. "Gemini CLI wants to enter plan mode"         → wants to enter
  if (/attention|approval|permission|needs your|wants to enter/i.test(message) && sessionId !== activeSessionId) {
    ...
    // "Gemini is waiting for your input" — delayed idle notification, mark response-ready
