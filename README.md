...
Your command center for Gemini CLI sessions.

SwitchboardGemini is a desktop app that gives you a unified view of all your Gemini CLI sessions across every project. Launch, resume, fork, and monitor sessions from a single window — no more juggling terminal tabs or digging through `~/.gemini/projects` to find that one conversation from last week.

![SwitchboardGemini](build/screenshot.png)

### Key Features

- **Session Browser** — All your Gemini CLI sessions, organized by project, searchable by content
...
- **IDE Emulation** — SwitchboardGemini acts as an IDE for Gemini CLI, showing file diffs and opens in a side panel where you can accept, reject, or edit changes before they're applied. Supports both inline and side-by-side diff views. Disable this in Global Settings if you prefer Gemini to use your own editor (VS Code, Cursor, etc.)
- **Plans & Memory** — Browse and edit your plan files and GEMINI.md memory in one place
- **Activity Stats** — Heatmap of your coding activity across all projects
- **Session Names** — Picks up session names from Gemini CLI's `/rename` command automatically
...
- **Live terminals** — Every open session renders its full terminal in a card, so you can monitor multiple Gemini agents simultaneously.
...
## File Preview Side Panel & Gemini IDE MCP Emulator

SwitchboardGemini can act as an IDE for your Gemini CLI sessions. When enabled, Gemini's file opens and proposed edits appear in a side panel next to the terminal instead of being sent to an external editor.

![IDE Emulation](build/screenshot-ide.png)

- **Diff review** — When Gemini proposes a file change, it shows up as a diff in the side panel. You can review the changes and accept or reject them directly.
...
To disable IDE emulation entirely (e.g. if you want Gemini to use VS Code or Cursor instead), uncheck **IDE Emulation** in **Global Settings**. This stops SwitchboardGemini from registering as an IDE, so Gemini CLI will discover and connect to your real editor. Changes take effect on new sessions — running sessions are not affected.

## Status Notifications

SwitchboardGemini monitors all your sessions in the background and shows status indicators in the sidebar so you can tell at a glance which sessions need attention — even when you're working in a different one.
...
- **Permission approval** — When Gemini is blocked waiting for a permission grant, the session badge lets you know immediately.
...
**[Download SwitchboardGemini](https://github.com/doctly/switchboard/releases/latest)**
