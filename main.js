...
const { fetchAndTransformUsage } = require('./gemini-auth');
...
const PROJECTS_DIR = path.join(os.homedir(), '.gemini', 'tmp', 'code', 'chats');
const PLANS_DIR = path.join(os.homedir(), '.gemini', 'tmp', 'code'); // Will be updated per session
const GEMINI_DIR = path.join(os.homedir(), '.gemini');
const STATS_CACHE_PATH = path.join(GEMINI_DIR, 'stats-cache.json');
...
function createWindow() {
  ...
  mainWindow.on('closed', () => {
    ...
    // Kill all running PTY processes so orphaned `gemini` processes don't
...
ipcMain.handle('add-project', (_event, projectPath) => {
  try {
    ...
    // Create the corresponding folder in ~/.gemini/projects/ so it persists
...
// --- IPC: read-plan ---
ipcMain.handle('read-plan', (_event, filename, sessionId) => {
  try {
    let filePath = path.join(PLANS_DIR, path.basename(filename));
    
    // If sessionId is provided, check session-specific plans folder
    if (sessionId) {
      const sessionPlansDir = path.join(PLANS_DIR, sessionId, 'plans');
      const sessionPath = path.join(sessionPlansDir, path.basename(filename));
      if (fs.existsSync(sessionPath)) filePath = sessionPath;
    }
    
    // Fallback search if not found
    if (!fs.existsSync(filePath)) {
       // Deep search in PLANS_DIR (which is ~/.gemini/tmp/code)
       const files = fs.readdirSync(PLANS_DIR, { withFileTypes: true });
       for (const f of files) {
         if (f.isDirectory()) {
           const p = path.join(PLANS_DIR, f.name, 'plans', path.basename(filename));
           if (fs.existsSync(p)) {
             filePath = p;
             break;
           }
    ...
    if (!fs.existsSync(filePath)) {
      return { content: '', filePath: '', error: 'File not found' };
    }
    ...
    return { content: '', filePath: '', error: err.message };
  }
});
...
ipcMain.handle('refresh-stats', async () => {
  ...
  // Helper: spawn gemini with args, collect output, auto-accept trust, kill when idle
  // waitFor: optional regex tested against stripped output — finish only when matched
  function runGemini(args, { timeoutMs = 15000, waitFor = null } = {}) {
    return new Promise((resolve) => {
      let output = '';
      let settled = false;
      let trustAccepted = false;
      // Track idle: ✳ in OSC title means Gemini is idle and waiting for input
      ...
      const geminiCmd = `gemini ${args}`;
      const p = pty.spawn(statsShell, shellArgs(statsShell, geminiCmd, statsShellExtraArgs), {
        name: 'xterm-256color',
        cols: 120,
        rows: 40,
        cwd: os.homedir(),
  ...
  try {
    ...
    const [, usage] = await Promise.all([
      runGemini('"/stats"', { waitFor: /streak/i, timeoutMs: 10000 }),
...
function folderToShortPath(folder) {
  // Convert "-Users-home-dev-MyGemini" → "dev/MyGemini"
...
ipcMain.handle('get-memories', () => {
  ...
  // --- Global files ---
  const globalFiles = scanMdFiles(GEMINI_DIR).map(f => ({ ...f, displayPath: '~/.gemini' }));
  ...
  try {
    if (fs.existsSync(PROJECTS_DIR)) {
      ...
      for (const folder of folders) {
        ...
        // Use same 2-deep short path as Sessions tab (e.g. "dev/MyGemini")
        ...
        // 1. ~/.gemini/projects/{folder}/ — gemini-home .md files
        const geminiHomeFiles = scanMdFiles(folderPath);
        for (const f of geminiHomeFiles) {
          files.push({ ...f, displayPath: '~/.gemini', source: 'gemini-home' });
        ...
        // 2. {projectPath}/ — project root GEMINI.md, agents.md
        if (projectPath) {
          for (const name of ['GEMINI.md', 'GEMINI.md', 'agents.md']) {
          ...
          // 3. {projectPath}/.gemini/ — commands/*.md and other .md files
          const dotGeminiDir = path.join(projectPath, '.gemini');
          const dotGeminiFiles = scanMdFiles(dotGeminiDir);
          for (const f of dotGeminiFiles) {
            if (!seenPaths.has(f.filePath)) {
              files.push({ ...f, displayPath: shortName + '/.gemini/', source: 'project' });
              seenPaths.add(f.filePath);
            }
          }
          // commands/*.md
          const commandsDir = path.join(dotGeminiDir, 'commands');
          const commandFiles = scanMdFiles(commandsDir);
          for (const f of commandFiles) {
            if (!seenPaths.has(f.filePath)) {
              files.push({ ...f, displayPath: shortName + '/.gemini/commands/', source: 'project' });
              seenPaths.add(f.filePath);
            }
          }
        }
...
ipcMain.handle('read-memory', (_event, filePath) => {
  try {
    const resolved = path.resolve(filePath);
    // Allow paths under ~/.gemini/ or any .md file that exists
    if (!resolved.endsWith('.md')) return '';
    if (!resolved.startsWith(GEMINI_DIR) && !fs.existsSync(resolved)) return '';
...
ipcMain.handle('read-session-jsonl', (_event, sessionId) => {
  ...
  let jsonlPath = path.join(PROJECTS_DIR, sessionId + '.jsonl');
  if (!fs.existsSync(jsonlPath) && folder) {
    // Try nested directory
    jsonlPath = path.join(PROJECTS_DIR, folder, sessionId + '.jsonl');
  }
  
  if (!fs.existsSync(jsonlPath)) return { error: 'Session not found on disk' };
...
ipcMain.handle('open-terminal', async (_event, sessionId, projectPath, isNew, sessionOptions) => {
  ...
  // WSL profiles only work for plain terminals — Gemini CLI sessions need the
  ...
  if (!isPlainTerminal) {
    ...
    const geminiProjectDir = path.join(PROJECTS_DIR, projectFolder);
    if (fs.existsSync(geminiProjectDir)) {
      try {
        knownJsonlFiles = new Set(
          fs.readdirSync(geminiProjectDir).filter(f => f.endsWith('.jsonl'))
        );
      } catch {}
    }
  ...
  try {
    if (isPlainTerminal) {
      // Plain terminal: interactive login shell, no gemini command
      // Inject a shell function to override `gemini` with a helpful message
      const geminiShim = 'gemini() { echo "\\033[33mTo start a Gemini session, use the + button in the sidebar.\\033[0m"; return 1; }; export -f gemini 2>/dev/null;';
      ...
      setTimeout(() => {
        if (!ptyProcess._isDisposed) {
          try {
            ptyProcess.write(geminiShim + ' clear\n');
          } catch {}
        }
      }, 300);
    } else {
      ...
      if (sessionOptions?.forkFrom) {
        ...
        geminiCmd = `gemini --resume "${sessionId}"`;
      }
      ...
      if (sessionOptions?.preLaunchCmd) {
        geminiCmd = sessionOptions.preLaunchCmd + ' ' + geminiCmd;
      }

      // Start MCP server for this session so Gemini CLI sends diffs/file opens to SwitchboardGemini
  ...
  const sessionPlansDir = path.join(os.homedir(), '.gemini', 'tmp', 'code', sessionId, 'plans');
  if (fs.existsSync(sessionPlansDir)) {
    const chokidar = require('chokidar');
    const planWatcher = chokidar.watch(sessionPlansDir, {
      ignoreInitial: true,
      persistent: true
    });
    planWatcher.on('add', (filePath) => {
      if (filePath.endsWith('.md')) {
        log.info(`[plan-watcher] New plan detected: ${filePath}`);
        if (mainWindow && !mainWindow.isDestroyed()) {
          mainWindow.webContents.send('plan-detected', sessionId, filePath);
        }
      }
    });
    session.planWatcher = planWatcher;
  }
  ...
  function processTerminalSignals(session, currentId, data) {
    ...
    if (data.includes('\x1b]')) {
      ...
      for (const m of oscMatches) {
        ...
        // Detect Gemini CLI busy state from OSC 0 title (spinner chars = busy, ✳ = idle)
        if (code === '0') {
  ...
  ptyProcess.onExit(({ exitCode, signal }) => {
    ...
    if (session.planWatcher) {
      session.planWatcher.close();
      session.planWatcher = null;
    }
