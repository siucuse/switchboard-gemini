...
function refreshFolder(folder) {
  ...
  for (const file of jsonlFiles) {
    ...
    if (s) {
      ...
      // Only customTitle (Gemini /title) promotes to session_meta.name — AI titles must NEVER
...
function buildProjectsFromCache(showArchived) {
  ...
  // Group by projectPath, not on-disk folder name. Multiple ~/.gemini/projects/<folder>/
  // directories can resolve to the same projectPath (Gemini CLI's folder-name encoding
